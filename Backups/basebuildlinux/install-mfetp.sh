...
# 6 - 64bit MFEcma 5.6.2-209 or above is required for installation to continue.
...
then
    ...
    echo "Usage: $0 [installtype] [oasoff] [usefanotify] [gtioff] [apoff] [apon] [nocontentupdate] [alttmppath=/home/installfolder]"
...
# Variable to indicate that current OS where product is being installed is RHEL 5.1x
isCurrentOsRhel5=0
...
checkForIncompatibleProduct()
{
    ...
    if [ "${DEB_VARIANT}" = "no" ]
    then
        rpm -q $productName > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            logMessage "Existing $productDesc is incompatible with McAfee Endpoint Security for Linux Threat Prevention 10.6.11-116. Aborting installation."
            restoreProcessStateAndAbort 19
        fi
    else
        dpkg -s $productName > /dev/null 2>&1
        if [ $? -eq 0 ]
        then
            logMessage "Existing $productDesc is incompatible with McAfee Endpoint Security for Linux Threat Prevention 10.6.11-116. Aborting installation."
            restoreProcessStateAndAbort 19
...
# Check if incompatible MACC is installed
# 1st param - supported MACC version
checkForIncompatibleMACC()
{
    maccVersion=$1
    if [ "${DEB_VARIANT}" = "no" ]
    then
        # In case of MACC, kmod is also present.So, perform sort and pick the first one.
        INSTALLED_MACC_VERSION_STRING=$(rpm -qa --queryformat "%{NAME}-%{VERSION}.%{RELEASE}\n" | grep -i solidcoreS3 | sort | head -1)
        if [ ! -z "${INSTALLED_MACC_VERSION_STRING}" ]
        then
            INSTALLED_MACC_VERSION_INT=$(echo "${INSTALLED_MACC_VERSION_STRING}" | awk -F '-' '{print $2}')
        fi
    else
        INSTALLED_MACC_VERSION_STRING=$(dpkg -s solidcoreS3 2>/dev/null  | grep ^Version | awk -F ': ' '{print $2}')
        if [ ! -z "${INSTALLED_MACC_VERSION_STRING}" ]
        then
            INSTALLED_MACC_VERSION_INT=$(echo "${INSTALLED_MACC_VERSION_STRING}" | sed -e 's:-:.:g')
        fi
    fi
    if [ ! -z "${INSTALLED_MACC_VERSION_STRING}" ] && [ "${forceInstallWithMACC}" == "no" ]
    then
        vercomp ${maccVersion} ${INSTALLED_MACC_VERSION_INT} 
        returnValue=$?
        if [ "${returnValue}" -eq 1 ] 
        then
            logMessage "Existing McAfee Solidifier for Linux ${INSTALLED_MACC_VERSION_INT} is incompatible with McAfee Endpoint Security for Linux Threat Prevention 10.6.11-116. Aborting installation."
            restoreProcessStateAndAbort 19
        fi
    fi
}
    ...
    case "${INSTALL_TYPE}" in
        ...
        "2")
            ...
            then
                ...
                if [ -d /var/McAfee ]
                then
                    checkSpace /var/McAfee 750000 24
                else
                    logMessage "MFEcma(x86_64) 5.6.2-209 or above is required for installation to continue."
                    cleanup 6
                fi
            fi
            ...
            then
                ...
                then
                    ...
                    retOfPkgMgrCmd=$?
                # To prevent over installation of VSEL on top of ENSL, ENSL uses "conflict" with VSEL in rpm spec
                # To perform automatic upgrade of VSEL to ENSL, ENSL uses "obsoletes" with VSEL in rpm spec
                # This combination of obsoletes and conflict works correctly with yum only for RHEL 6 and above
                # The workaround is to install ESP and Kernel modules using yum and to use rpm with "U" to install ENSL
                # NOTE: rpm with "i" has the same issue that yum does, so "U" is used instead.
                # This work around is being done only in case of a upgrade from VSEL to ENSL on a RHEL 5.10/5.11 system
                elif [ "${vselInstalled}" -eq 1 ] && [  ${isCurrentOsRhel5} -eq 1 ]
                then
                    logMessage "Upgrading VSEL to ENSL on RHEL 5.1x"
                    # Run install by disabling all repo to prevent checking in external site
                    # This particular section installs FMP-Lite related packages using yum command
                    yum -y --nogpgcheck --noplugins --disablerepo=* install ${pkgMgrDir}/McAfeeRt*.rpm ${pkgMgrDir}/McAfeeESP*.rpm
                    retOfPkgMgrCmd=$?
                    # Install McAfeeTP package separately only if Runtime, ESP and Kernel modules was successfully installed
                    if [ ${retOfPkgMgrCmd} -eq 0 ]
                    then
                        rpm -Uv ${pkgMgrDir}/McAfeeTP*.rpm
                        retOfPkgMgrCmd=$?
                    fi
                else
...
# This checks are exactly same as the one kept in preinst of dpkg and %pre in the rpm.spec
checkForUnSupportedDistro()
{
    ...
    then
        ...
        # If version is less than minimum supported version for known distribution, then bail out
        if [ -n "${distribRelString}" ]
        then
            if [ "${distribRelString}" = "ubuntu" -a "${distribRelMajNum}" -lt "${minUbuntuSupport}" ]
            then
                echo "McAfeeTP is not supported on this Ubuntu version - ${distribRel}"
                echo "Product is unsupported on this distribution. Please see KB87073."
                cleanup 19
            elif [ "${distribRelString}" = "debian" -a "${distribRelMajNum}" -lt "${minDebianSupport}" ]
            then
                echo "McAfeeTP is not supported on this Debian version - ${distribRel}"
                echo "Product is unsupported on this distribution. Please see KB87073."
                cleanup 19
            fi
        else
        ...
        opensuseSearchPattern="^openSUSE"
        ...
        then
            # RHEL 5.10 and above is supported
            if ( [ ${distribRelMajNum} -ge 6 ] ) || ( [ ${distribRelMajNum} -eq 5 ] && [ ${distribRelMinNum} -ge 10 ] )
            then
        ...
        then
            # Suse 11 and above is supported
            if [ ${distribRelMajNum} -ge 11 ]
            then
                isSupported="yes"
            fi
        ...
        then
            # All openSUSE is currently considered to be supported
            isSupported="yes"
        elif [[ $distribRelString =~ $amazonSearchPattern ]]
        then
            # Amazon 2014 and above is supported
            if [ ${distribRelMajNum} -ge 2014 ]
            then
                isSupported="yes"
            fi
        ...
        then
            # Fedora 21 and above is supported
            if [ ${distribRelMajNum} -ge 21 ]
            then
                isSupported="yes"
            fi
        else
...
# Function to detect if current OS is RHEL 5.1x
isRhel51xVariant()
{
    isCurrentOsRhel5=0
    if [ -f "/etc/redhat-release" ]
    ...
    # Enable case insensitive match
    shopt -s nocasematch
    if [[ $distribRelString =~ "Red_Hat_Enterprise_Linux_Server_release_5*" ]]
    then
        isCurrentOsRhel5=1
    fi
}
...
PACKAGED_MFE_TP_VERSION_INT="10.6.11.116"
...
installerFile="${currentDir}/McAfeeTP-10.6.11-116-standalone.linux.tar.gz"
...
# Install only if MACC 6.4.1 or later is present
checkForIncompatibleMACC 6.4.1
...
then
    ...
    then 
        ...
        checkForIncompatibleProduct "mcafeemoveagntls" "MOVE AntiVirus"
    fi
fi
...
then
    ...
    MFE_TP_PACKAGE_FILE="${tmpDir}/McAfeeTP-10.6.11-116.deb"
else
    logMessage "Detected rpm based distribution"
    UNINSTALL_CMD="rpm -e "
    # Format is McAfeeTP-8.0.0-1600.x86_64.rpm
    MFE_TP_PACKAGE_FILE="${tmpDir}/McAfeeTP-10.6.11-116.x86_64.rpm"
fi
...
# Only for a fresh installation, invoke ESP installer script to install runtime, ESP, ESP for FileAccess and ESP Arbitrary Access Control
    ...
    if [ -f ${tmpDir}/McAfeeESP-KernelModule-10.6.11-128-Full.linux.tar.gz ]
    then
        bash ${tmpDir}/validate-mfeesp.sh ${installType} ${log} ${pkgMgrDir} ${tmpDir}/McAfeeESP-Basic-10.6.11-128-Full.linux.tar.gz ${tmpDir}/McAfeeESP-KernelModule-10.6.11-128-Full.linux.tar.gz
    else
        bash ${tmpDir}/validate-mfeesp.sh ${installType} ${log} ${pkgMgrDir} ${tmpDir}/McAfeeESP-Basic-10.6.11-128-Full.linux.tar.gz ${currentDir}/McAfeeESP-KernelModule-10.6.11-128-Full.linux.tar.gz
    fi
...
then
    ...
    /opt/McAfee/ens/runtime/2.0/lib/ldconfig -C /etc/ld-mfeensrt-2.0.so.cache -f /etc/ld-mfeensrt-2.0.so.conf >/dev/null 2>&1
